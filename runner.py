...
def play():
    ...
    if zero_player_game:
        all_remaining = [list(range(1, 49) if mode == 2 else list(range(1, 24))) for _ in range(players)]
        initial_spaces_out = 108
        all_possible_spaces = {}
        ...
        print(total_comb(all_remaining))
        if total_comb(all_remaining) < 130000:
            ...
            for possible in itertools.product(*all_remaining):
                ...
                if len(working_spaces) == 1 and len(set(possible)) == len(possible):
                    ...
                    for i, p in enumerate(possible):
                        all_possibilities[i].add(p)
            # Update the possible remaining with only the clues that led to once space
            all_remaining = [list(o) for o in all_possibilities]
            print(all_remaining)
        ...
        try:
            ...
            initial_spaces_out = len(non_satisfied_spaces)
